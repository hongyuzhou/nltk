...
from ctypes import (c_bool, c_char, c_char_p, c_double, c_int, c_uint,
                    c_ulong, c_void_p, cdll, POINTER, Structure, byref, cast)
import logging
import os
import sys
...
def segment_pos(s):
    size = c_int()
    result = ParagraphProcessA(s, byref(size), True)
    result_t_vector = cast(result, POINTER(ResultT))
    words = []
    for i in range(0, size.value):
        r = result_t_vector[i]
        word = s[r.start:r.start+r.length]
        words.append((word.decode('utf-8'), r.sPOS.decode('utf-8')))
    return words

def import_userdict(f):
    count = ImportUserDict(f)
    print "import %s words" % count